...
public class RewriteMe {
    ...
    RewriteMe(){
        getLongestLettercountAnwerInAGivenCategory(Category.FOOD);
    }
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category){
       return (int) questions.stream().map(Question::getCategory).filter(s -> s.equals(category)).count();
    ...
    public List<String> getAllAnswerOptionsDistinct(){
        List<List<String>> l = questions.stream().map(Question::getAllAnswers).collect(Collectors.toList());

        return l.stream().flatMap(list -> list.stream()).distinct().collect(Collectors.toList());
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        List<String> l = questions.stream().map(Question::getAllAnswers).flatMap(list -> list.stream()).collect(Collectors.toList());

        return l.stream().anyMatch(s -> s.equals(answerCandidate));
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        List<String> l = questions.stream().map(Question::getAllAnswers).flatMap(list -> list.stream()).collect(Collectors.toList());
        return (int) l.stream().filter(s -> s.equals(answerCandidate)).count();
    }
    ...
    public String getLongestLettercountAnwerInAGivenCategory(Category c)
    {
        return questions.stream().filter(s -> s.getCategory().equals(c))
                .map(Question::getAllAnswers).
                        ...
                        stream().reduce((s1,s2)->s1.length() >= s2.length() ? s1 : s2).get();
